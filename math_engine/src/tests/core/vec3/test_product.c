...
static void bench_vec3_dot(void)
{
	...
	printf("vec3_dot: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_vec3_cross(void)
{
	...
	double sec = time_diff_sec(t1, t2);
	printf("vec3_cross: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_vec3_volume(void)
{
	volatile float result;
	t_vec3 a = vec3_new(1.0f, 2.0f, 3.0f);
	t_vec3 b = vec3_new(4.0f, 5.0f, 6.0f);
	...
	double sec = time_diff_sec(t1, t2);
	printf("vec3_volume: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_vec3_triple(void)
{
	volatile t_vec3 result;
	t_vec3 a = vec3_new(1.0f, 2.0f, 3.0f);
	t_vec3 b = vec3_new(4.0f, 5.0f, 6.0f);
	...
	clock_gettime(CLOCK_MONOTONIC, &t1);
	for (int i = 0; i < TEST_ITERATIONS; i++)
		result = vec3_triple(a, b, c);
	clock_gettime(CLOCK_MONOTONIC, &t2);

	double sec = time_diff_sec(t1, t2);
	printf("vec3_triple: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_product_mixed(void)
{
	...
	double sec = time_diff_sec(t1, t2);
	printf("mixed product: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)dot_result;
	(void)cross_result;
}
