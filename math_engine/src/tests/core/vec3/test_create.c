...
static void bench_vec3_new(void)
{
	...
	printf("vec3_new: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_vec3_from_scalar(void)
{
	volatile t_vec3 result;
	...
	double sec = time_diff_sec(t1, t2);
	printf("vec3_from_scalar: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_create_mixed(void)
{
	volatile t_vec3 result;
	...
	double sec = time_diff_sec(t1, t2);
	printf("mixed create: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}
...
{
#ifdef BENCHMARK
	printf("\n=== Running benchmarks (create) ===\n");
	bench_vec3_new();
	bench_vec3_from_scalar();
	bench_create_mixed();
#else
	srand(42);
