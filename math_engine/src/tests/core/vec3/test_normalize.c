...
static void benchmark_vec3_normalize(void)
{
	...
	printf("vec3_normalize: %d ops in %.3f s -> %.1f Mops/s\n",
...
static void benchmark_vec3_normalize_or(void)
{
	...
	double sec = time_diff_sec(t1, t2);
	printf("vec3_normalize_or: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}
...
static void benchmark_vec3_is_normalized(void)
{
	...
	double sec = time_diff_sec(t1, t2);
	printf("vec3_is_normalized: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}
...
static void benchmark_normalize_mixed(void)
{
	...
	double sec = time_diff_sec(t1, t2);
	printf("mixed normalize: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)vec_result;
	(void)bool_result;
}
...
{
#ifdef BENCHMARK
    printf("\n=== Running benchmarks (normalization) ===\n");
