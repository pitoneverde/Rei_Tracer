...
static void bench_vec3_project(void)
{
	...
	printf("vec3_project: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_vec3_reject(void)
{
	volatile t_vec3 result;
	t_vec3 v = vec3_new(1.0f, 2.0f, 3.0f);
	t_vec3 onto = vec3_new(0.0f, 1.0f, 0.0f);
	...
	double sec = time_diff_sec(t1, t2);
	printf("vec3_reject: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}
...
static void bench_vec3_distance(void)
{
	...
	double sec = time_diff_sec(t1, t2);
	printf("vec3_distance: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}
...
static void bench_vec3_distance_sq(void)
{
	...
	double sec = time_diff_sec(t1, t2);
	printf("vec3_distance_sq: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}
...
static void bench_project_mixed(void)
{
	...
	for (int i = 0; i < TEST_ITERATIONS; i++)
	{
		...
		switch (i & 7)
		{
			case 0: case 1:  // 25% distance
				float_result = vec3_distance(a, b);
				break;
			case 2: case 3:  // 25% distance_sq
				float_result = vec3_distance_sq(a, b);
				break;
			case 4: case 5:  // 25% project
				vec_result = vec3_project(a, b);
				break;
			case 6: case 7:  // 25% reject
	...
	double sec = time_diff_sec(t1, t2);
	printf("mixed project: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)vec_result;
	(void)float_result;
}
...
{
#ifdef BENCHMARK
	printf("\n=== Running benchmarks (project/distance) ===\n");
	bench_vec3_project();
	bench_vec3_reject();
	bench_vec3_distance();
	bench_vec3_distance_sq();
	bench_project_mixed();
#else
	srand(42);
