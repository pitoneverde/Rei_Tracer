...
static void bench_vec3_equal(void)
{
	...
	printf("vec3_equal: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_vec3_is_zero(void)
{
	volatile bool result;
	...
	double sec = time_diff_sec(t1, t2);
	printf("vec3_is_zero: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_vec3_is_finite(void)
{
	volatile bool result;
	t_vec3 v = vec3_new(1.0f, 2.0f, 3.0f);
	...
	double sec = time_diff_sec(t1, t2);
	printf("vec3_is_finite: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_vec3_any_nan(void)
{
	volatile bool result;
	t_vec3 v = vec3_new(1.0f, 2.0f, 3.0f);
	...
	clock_gettime(CLOCK_MONOTONIC, &t1);
	for (int i = 0; i < TEST_ITERATIONS; i++)
		result = vec3_any_nan(v);
	clock_gettime(CLOCK_MONOTONIC, &t2);

	double sec = time_diff_sec(t1, t2);
	printf("vec3_any_nan: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_vec3_abs(void)
{
	...
	double sec = time_diff_sec(t1, t2);
	printf("vec3_abs: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_vec3_floor(void)
{
	volatile t_vec3 result;
	t_vec3 v = vec3_new(-1.0f, 2.0f, -3.0f);
	...
	double sec = time_diff_sec(t1, t2);
	printf("vec3_floor: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_vec3_ceil(void)
{
	volatile t_vec3 result;
	t_vec3 v = vec3_new(-1.0f, 2.0f, -3.0f);
	...
	double sec = time_diff_sec(t1, t2);
	printf("vec3_ceil: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_vec3_round(void)
{
	volatile t_vec3 result;
	t_vec3 v = vec3_new(-1.0f, 2.0f, -3.0f);
	...
	double sec = time_diff_sec(t1, t2);
	printf("vec3_round: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_vec3_mod(void)
{
	volatile t_vec3 result;
	t_vec3 v = vec3_new(-1.0f, 2.0f, -3.0f);
	float divisor = 2.5f;
	...
	clock_gettime(CLOCK_MONOTONIC, &t1);
	for (int i = 0; i < TEST_ITERATIONS; i++)
		result = vec3_mod(v, divisor);
	clock_gettime(CLOCK_MONOTONIC, &t2);

	double sec = time_diff_sec(t1, t2);
	printf("vec3_mod: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_compare_mixed(void)
{
	...
	double sec = time_diff_sec(t1, t2);
	printf("mixed compare: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)bool_result;
	(void)vec_result;
}
...
{
#ifdef BENCHMARK
	printf("\n=== Running benchmarks (comparisons/utils) ===\n");
	bench_vec3_equal();
	bench_vec3_is_zero();
	bench_vec3_is_finite();
	bench_vec3_any_nan();
	bench_vec3_abs();
	bench_vec3_floor();
	bench_vec3_ceil();
