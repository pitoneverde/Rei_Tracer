...
static void bench_vec3_angle(void)
{
	...
	printf("vec3_angle: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_vec3_signed_angle(void)
{
	volatile float result;
	t_vec3 a = vec3_new(1.0f, 0.0f, 0.0f);
	t_vec3 b = vec3_new(0.0f, 1.0f, 0.0f);
	...
	double sec = time_diff_sec(t1, t2);
	printf("vec3_signed_angle: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_vec3_is_parallel(void)
{
	...
	double sec = time_diff_sec(t1, t2);
	printf("vec3_is_parallel: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_vec3_is_perpendicular(void)
{
	volatile bool result;
	t_vec3 a = vec3_new(1.0f, 0.0f, 0.0f);
	t_vec3 b = vec3_new(0.0f, 1.0f, 0.0f);
	...
	double sec = time_diff_sec(t1, t2);
	printf("vec3_is_perpendicular: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)result;
}

static void bench_angles_mixed(void)
{
	...
	double sec = time_diff_sec(t1, t2);
	printf("mixed angle: %d ops in %.3f s -> %.1f Mops/s\n",
		   TEST_ITERATIONS, sec, TEST_ITERATIONS / sec / 1e6);
	(void)float_result;
	(void)bool_result;
}
...
{
#ifdef BENCHMARK
    printf("\n=== Running benchmarks (angles/direction) ===\n");
	bench_vec3_angle();
	bench_vec3_signed_angle();
	bench_vec3_is_parallel();
	bench_vec3_is_perpendicular();
	bench_angles_mixed();
#else
	srand(42);
