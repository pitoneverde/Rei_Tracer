...
static void bench_vec3_coordinate_system(void)
{
	...
	for (int i = 0; i < TEST_ITERATIONS; i++)        
		vec3_coordinate_system(a, (t_vec3*)&b_result, (t_vec3*)&c_result);
	clock_gettime(CLOCK_MONOTONIC, &t2);

	double sec = time_diff_sec(t1, t2);
	printf("vec3_coordinate_system: %d ops in %.3f s -> %.1f Mops/s\n",
